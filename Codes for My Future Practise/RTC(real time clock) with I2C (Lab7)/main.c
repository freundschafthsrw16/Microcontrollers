...
*/
#define DS1307_I2C_ADR 0xD0             //  Address of the DS1307
...
void display_showtime(void){
	...
	if (weekday < 9 && weekday > 0 ){
		snprintf(str, 16, "%02d.%02d  %s", day, month, dayofweek[weekday - 1 ]);
	} else {
...
uint8_t ds1307_getTime(void){
	...
	// Read value
	second = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);      // Convert BCD byte to Binary for variable second
	// TODO minute, hour, ...
        minute = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);     // Convert BCD byte to Binary for variable minute
	hour = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);       // Convert BCD byte to Binary for variable hour
	weekday = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);   // Convert BCD byte to Binary for variable weekday
	day = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);       // Convert BCD byte to Binary for variable day
    month = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);        // Convert BCD byte to Binary for variable month
	year = ds1307_decodeBcd(i2c_master_read_last() & 0x7F);     // Convert BCD byte to Binary for variable year and finish the I2C task
...
int main(void){
	...
	while (1){
		...
		ds1307_getTime();               // load the current time
