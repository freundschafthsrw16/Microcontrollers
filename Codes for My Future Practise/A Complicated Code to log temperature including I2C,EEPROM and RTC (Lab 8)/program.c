...
void ds1307_SetToDefaultTime(void){
	...
	month = 12;
	year = 18;
	// Write this time to the DS1307
	ds1307_setTime();
}
/****************************************************************/
void recording(uint16_t t,uint16_t no_values)
{
	uint16_t x;
	char str[16];
	lcd_clear();
...
*/
int main(void){
	uint16_t nowtemp,memory_occupied;
	...
	for(;;){
		...
		// Show recorded data
		 if ( ~PINB & (1 << PB1) )
		 {
			 while(~PINB & (1 << PB1) )
				 _delay_ms(50);
			memory_occupied = load_value8bit(0);
			 if(memory_occupied==0)
				no_data();
			 else
					 show_data();
		 }
